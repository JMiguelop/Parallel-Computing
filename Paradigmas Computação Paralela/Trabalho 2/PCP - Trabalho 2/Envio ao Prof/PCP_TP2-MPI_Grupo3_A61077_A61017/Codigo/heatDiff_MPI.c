...
void iterate(float **M_New, float **M_Old, int N, int offset, int numLines) {
	int startRow;
	int endRow;
...
int main(int argc, char *argv[]) {
	...
	if(procID != MASTER) {
		...
		for(int iteration = 0; iteration < n_max_iterations; iteration++) {	
			...
			iterate(heatPlate_new, heatPlate_old, matrix_size, offset, numLines);
